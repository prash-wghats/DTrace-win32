...
dt_provmod_open(dt_provmod_t **provmod, dt_fdlist_t *dfp)
{
	...
	for (i = 0; i < MAX_PROVIDER; i++) {
		...
		if ((fd = CreateFile(providers[i], GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL)) == NULL)
			return;
		if (((prov = malloc(sizeof (dt_provmod_t))) == NULL) ||
		    (prov->dp_name = malloc(strlen(providers[i]) + 1)) == NULL) {
			free(prov);
			CloseHandle(fd);
			return;
		}

		(void) strcpy(prov->dp_name, providers[i]);
...
#else
	dtfd = CreateFile("\\\\.\\Dtrace", GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
#endif
...
#else
	ftfd = CreateFile("\\\\.\\Fasttrap", GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
