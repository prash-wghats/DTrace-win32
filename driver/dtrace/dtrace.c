...
#else
static PETHREAD		dtrace_panicked;
#endif
...
#else
static KMUTEX		dtrace_lock;		/* probe state lock */
static KMUTEX		dtrace_provider_lock;	/* provider state lock */
static KMUTEX		dtrace_meta_lock;	/* meta-provider state lock */
static KMUTEX		cpu_lock;	
#endif
/*
...
#define	DTRACE_TLS_THRKEY(where) { \
	uint_t intr = 0; \
	uint_t actv = KeGetCurrentIrql() >> (LOCK_LEVEL + 1); \
	for (; actv; actv >>= 1) \
		intr++; \
	ASSERT(intr < (1 << 3)); \
	(where) = (((uint64_t)PsGetCurrentThreadId() + DIF_VARIABLE_MAX) & \
	    (((uint64_t)1 << 61) - 1)) | ((uint64_t)intr << 61); \
}
#endif
...
#define	DTRACE_ALIGNCHECK(addr, size, flags)				\
	if (addr & (size - 1)) {					\
		*flags |= CPU_DTRACE_BADALIGN;				\
		cpu_core[KeGetCurrentProcessorNumber()].cpuc_dtrace_illval = addr;	\
		return (0);						\
	}
#else
#define	DTRACE_ALIGNCHECK(addr, size, flags)
...
{									\
	...
	volatile uint16_t *flags = (volatile uint16_t *)		\
		&cpu_core[KeGetCurrentProcessorNumber()].cpuc_dtrace_flags;	\
									\
...
#else
	volatile uintptr_t *illval = &cpu_core[KeGetCurrentProcessorNumber()].cpuc_dtrace_illval;
#endif 
...
#else
	flags = (volatile uint16_t *)&cpu_core[KeGetCurrentProcessorNumber()].cpuc_dtrace_flags;
#endif
	do {
		if (s1 == NULL) {
			c1 = '\0';
...
#else
			cpu_core[KeGetCurrentProcessorNumber()].cpuc_dtrace_illval = kaddr;
#endif 
			return (1);
		}
...
#else
			cpu_core[KeGetCurrentProcessorNumber()].cpuc_dtrace_illval = taddr;
#endif 
			return (1);
		}
	}
...
#else
	cpu_core[KeGetCurrentProcessorNumber()].cpuc_dtrace_flags |= CPU_DTRACE_UPRIV;
#endif 
	return (0);
}
...
#else
	processorid_t me = KeGetCurrentProcessorNumber(), cpu = me;
#endif	
...
#else
	processorid_t cpu = KeGetCurrentProcessorNumber();
#endif	
    ...
    uint64_t ndx)
{
	...
	case DIF_VAR_UREGS: {
		...
		if ((regs = curthread->tf) == NULL) {
			DTRACE_CPUFLAG_SET(CPU_DTRACE_BADADDR);
			cpu_core[KeGetCurrentProcessorNumber()].cpuc_dtrace_illval = 0;
			return (0);
		}
...
#else
		return ((uint64_t)(uintptr_t)PsGetCurrentThread());
#endif
...
#else
		return ((uint64_t)curthread->p_pid); 
#endif
	case DIF_VAR_TID:
#if defined(sun)
		/*
    ...
    dtrace_mstate_t *mstate, dtrace_state_t *state)
{
	...
	volatile uintptr_t *illval = &cpu_core[CPU->cpu_id].cpuc_dtrace_illval;
#else
	volatile uint16_t *flags = &cpu_core[KeGetCurrentProcessorNumber()].cpuc_dtrace_flags;
	volatile uintptr_t *illval = &cpu_core[KeGetCurrentProcessorNumber()].cpuc_dtrace_illval;
#endif
	...
	while (pc < textlen && !(*flags & CPU_DTRACE_FAULT)) {
		...
		case DIF_OP_LDLS:
			...
			if (v->dtdv_type.dtdt_flags & DIF_TF_BYREF) {
				...
				a += CPU->cpu_id * sz;
#else
				a += KeGetCurrentProcessorNumber() * sz;
#endif
				if (*(uint8_t *)a == UINT8_MAX) {
					/*
					 * If the 0th byte is set to UINT8_MAX
...
#else
			regs[rd] = tmp[KeGetCurrentProcessorNumber()];
#endif		
			break;
...
#else
			tmp[KeGetCurrentProcessorNumber()] = regs[rd];
#endif		
			break;
...
#else
	if (dtrace_casptr(&dtrace_panicked, NULL, PsGetCurrentThread()) != NULL)
#endif		
		return;
...
#else	
	uint16_t *flags = &cpu_core[KeGetCurrentProcessorNumber()].cpuc_dtrace_flags;
#endif	
	char *sym;
    ...
    cred_t *cr, const dtrace_pops_t *pops, void *arg, dtrace_provider_id_t *idp)
{
...
#else
		DbgPrint("failed to register provider '%s': invalid arguments", 
		    name ? name : "<NULL>");
#endif		
		return (EINVAL);
	}
	...
	if (name[0] == '\0' || dtrace_badname(name)) {
		...
		cmn_err(CE_WARN, "failed to register provider '%s': invalid "
		    "provider name", name);
#else
		DbgPrint("failed to register provider '%s': invalid provider name", name);
#endif		
		return (EINVAL);
	}
	...
	if ((pops->dtps_provide == NULL && pops->dtps_provide_module == NULL) ||
		...
		cmn_err(CE_WARN, "failed to register provider '%s': invalid "
		    "provider ops", name);
#else
		DbgPrint("failed to register provider '%s': invalid provider ops", name);
#endif		
		return (EINVAL);
	}
...
#else
		DbgPrint("failed to register provider '%s': invalid provider attributes", name);
#endif		
		return (EINVAL);
	}
...
#else
		DbgPrint("failed to register provider '%s': invalid privilege attributes", name);
#endif		
		return (EINVAL);
	}
...
#else
		DbgPrint("failed to register provider '%s': need dtps_usermode()"
		" op for given privilege attributes", name);
#endif		
		return (EINVAL);
	}
	...
	*idp = DTRACE_METAPROVNONE;
	...
	if (name == NULL) {
		...
		cmn_err(CE_WARN, "failed to register meta-provider: "
		    "invalid name");
#else
		DbgPrint("failed to register meta-provider: invalid name");
#endif		
		return (EINVAL);
	}
	...
	if (mops == NULL ||
		...
		cmn_err(CE_WARN, "failed to register meta-register %s: "
		    "invalid ops", name);
#else		
		DbgPrint("failed to register meta-register %s: invalid ops", name);	
#endif		
		return (EINVAL);
	}
...
#else
		DbgPrint("failed to register meta-register %s: user-land meta-provider exists", name);
#endif	
...
#else
		DbgPrint("dtrace DIF object error: [%u]: ", pc);
#endif		
		va_start(alist, format);
#if defined(sun)
		(void) vuprintf(format, alist);
#else
		RtlStringCbVPrintfA(str, 256, format, alist);
		DbgPrint(str);
#endif		
		va_end(alist);
	}
...
#else
	buf = &state->dts_buffer[KeGetCurrentProcessorNumber()];
#endif
...
#else
		DbgPrint("enabling probe %d (%s:%s:%s:%s)", i, desc->dtpd_provider, 
		...
		if (enab->dten_error != 0) {
			...
			if (nmatched == NULL) {
				...
				cmn_err(CE_WARN, "dtrace_enabling_match() "
				    ...
				    enab->dten_error);
#else
				DbgPrint("dtrace_enabling_match() error on %p: %d", 
...
#else
		DbgPrint("failed to process DOF: %s", str);
#endif
#ifdef DTRACE_ERRDEBUG
	dtrace_errdebug(str);
#endif
...
#if !defined(sun)
	//callout_reset(&state->dts_cleaner, state->dts_options[DTRACEOPT_CLEANRATE]);
#endif
}
static void
...
#else
dtrace_state_create(PDEVICE_OBJECT dev, void *p)
#endif
{
#if defined(sun)
	minor_t minor;
...
#else
	RtlStringCchPrintfA(c, sizeof(c), "dtrace_aggid_%d", 0);
#endif	
...
dtrace_anon_property(void)
{
	...
	for (i = 0; ; i++) {
...
#else
		RtlStringCchPrintfA(c, sizeof (c), "dof-data-%d", i);
#endif
		...
		if (kdi_dtrace_set(KDI_DTSET_DTRACE_ACTIVATE) != 0) {
			...
			DbgPrint(CE_NOTE, "kernel debugger active; anonymous enabling ignored.");
		...
		if ((state = dtrace_anon.dta_state) == NULL) {
			...
			if (state == NULL) {
				...
				cmn_err(CE_WARN, "failed to create "
				    "anonymous state");
#else
				DbgPrint("failed to create anonymous state");
#endif
				dtrace_dof_destroy(dof);
				break;
			}
...
#else
	ent->dtht_illval = cpu_core[KeGetCurrentProcessorNumber()].cpuc_dtrace_illval;
#endif
...
#else
		ent->dtht_locals[i] = ((uint64_t *)(uintptr_t)svar->dtsv_data)[KeGetCurrentProcessorNumber()];		
#endif
	}
}
...
#else
				DbgPrint( "unloaded module '%s' had enabled probes", 
				    ctl->mod_modname);
#endif		
			}
 ...
 */
/*ARGSUSED*/
int dtrace_open(PDEVICE_OBJECT dev, void *state)
...
dtrace_load(void *dummy)
{
	...
	cpu_core = kmem_zalloc(sizeof(cpu_core_t) * KeNumberProcessors, KM_SLEEP);
	CPU = kmem_zalloc(sizeof(cpu_data_t) * KeNumberProcessors, KM_SLEEP);
#if defined(freebsd)	
...
#if defined(windows)
		DbgPrint("illegal value (%lu) for dtrace_retain_max; setting to 1 ", 
		    dtrace_retain_max);
#else
...
int
dtrace_unload(PDRIVER_OBJECT DrvObj)
{
	dtrace_state_t *state;
	int error = 0;
...
dtrace_ioctl_helper(int cmd, intptr_t arg, int *rv)
{
	...
	case DTRACEHIOC_ADD: {
		...
		if ((uint32_t) p->pid == (uint32_t) dhp->dofhp_pid) {
			dof = dtrace_dof_copyin(arg, &rval);
		 }else {
...
/* ARGSUSED */
int
dtrace_ioctl(void *addr,  int cmd, void *ext, int len, void *oaddr)
{
	dtrace_state_t *state;
	int rval;
