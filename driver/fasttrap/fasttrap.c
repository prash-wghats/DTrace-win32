...
static KMUTEX fasttrap_cleanup_mtx;
...
#else
static KMUTEX			fasttrap_count_mtx;	/* lock on ref count */
#endif
...
#if !defined(sun)
static KMUTEX *fasttrap_cpuc_pid_lock;
#endif
...
#else
	if (RtlStringCchPrintfA(provname, sizeof (provname), "%s%u", name, (uint_t)pid) >=
#endif	    
	    sizeof (provname) ||
...
#if defined(sun)
		printf("failed to instantiate provider %s: name too long to accomodate pid", dhpv->dthpv_provname);
#else
		DbgPrint("failed to instantiate provider %s: name too long to accomodate pid", dhpv->dthpv_provname);
#endif
		return (NULL);
	}
...
#if defined(sun)
		printf("failed to instantiate provider %s: %s is an invalid name", dhpv->dthpv_provname, FASTTRAP_PID_NAME);
#else
		DbgPrint("failed to instantiate provider %s: %s is an invalid name", dhpv->dthpv_provname, FASTTRAP_PID_NAME);
#endif		
		return (NULL);
	}
...
#if defined(sun)		
		printf("failed to instantiate provider %s for process %u",  dhpv->dthpv_provname, (uint_t)pid);
#else
		DbgPrint("failed to instantiate provider %s for process %u",  dhpv->dthpv_provname, (uint_t)pid);
#endif		
		return (NULL);
	}
...
/*ARGSUSED*/
int
fasttrap_ioctl(void *arg,  int cmd, int len)
{
#ifdef notyet
...
fasttrap_load(void)
{
	...
	fasttrap_cpuc_pid_lock = kmem_zalloc(MAXCPU * sizeof(KMUTEX), KM_SLEEP);
	CPU_FOREACH(i) {
